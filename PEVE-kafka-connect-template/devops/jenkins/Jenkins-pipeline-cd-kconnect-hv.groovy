...
try {
  node{
    ...
    sh "mkdir -p ${WORKSPACE}/smt"
    if(transformer_url != '') {
        println "Descargando SMT jar"
        smt_jar_name = transformer_url.substring(transformer_url.lastIndexOf("/") + 1, transformer_url.length())
        sh "echo smt_jar_name = ${smt_jar_name}"
        sh "echo transformer_url = ${transformer_url}"
        sh "curl ${transformer_url} --output smt/${smt_jar_name}"
        println "Terminado SMT jar "      
    }
    ...
    stage('Deploy connector to ' + deploymentEnvironment){
      utils.setDeltaInit()
        ...
        def KAFKA_KEYSTORE = "${kafkaConnectName}-keystore.jks"
        def KAFKA_TRUSTSTORE = "${kafkaConnectName}-truststore.jks"
        ...
        def globals = readYaml file: "${WORKSPACE}/devops/deploy/global-vars.yaml"
        ...
        def ansible_cmd = "ansible-playbook ${WORKSPACE}/devops/ansible/site.yaml -v -i ${WORKSPACE}/devops/ansible/hosts.yml \
      ...
      utils.withHashicorpVaultCredentialAtla(hosted_on_environment[hosted].credentials, deploymentEnvironment) {
        utils.dockerWithRegistryHashicorpVaultAtla("${DOCKER_SUPPORT_REGISTRY_CREDENTIAL}", "${DOCKER_SUPPORT_REGISTRY_URL}") {
          ...
          if (hosted == 'ocp') {
            ansible_cmd_extra << [azure_registry_username: "${AZURE_REGISTRY_USERNAME}",
              azure_registry_password: "${AZURE_REGISTRY_PASSWORD}"]
          }
          ansible_cmd_extra << [azure_docker_registry: "${containerRegistry}",
