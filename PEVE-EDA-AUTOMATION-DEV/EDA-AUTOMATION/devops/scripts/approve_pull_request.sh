...
RESPONSE_FILE="approve_response.json"
HTTP_CODE=$(
  curl -sS -X POST \
    -H "Authorization: token ${TOKEN}" \
    -H "Accept: application/vnd.github+json" \
    "https://api.github.com/repos/${OWNER_REPO}/pulls/${NUM_PR}/reviews" \
    -d "{
      \"event\": \"APPROVE\",
      \"body\": \"${JIRA_ISSUE} Aprobado automáticamente por CI\"
    }" \
    -w "%{http_code}" \
    -o "${RESPONSE_FILE}"
)
...
if [[ "${HTTP_CODE}" == "200" || "${HTTP_CODE}" == "201" ]]; then
  echo "✅ Approve realizado correctamente. COMMIT_ID : ${COMMIT_ID}"
  exit 0
else
