#!/bin/bash
set -euo pipefail
source ./devops/scripts/terraform_custom_plan_guard.sh
...
#		chmod +x ./automation/scripts/generate_rbac_dinamic.sh
		chmod +x ./devops/scripts/generate_rbac_dinamic.sh
		./devops/scripts/generate_rbac_dinamic.sh "${NAME_DIR_APP}" "${RELATIVE_SUBPATH_SRC}" "$baseFilename" "${ENV}"

	  else
#		chmod +x ./automation/scripts/gen_topic_dinamic.sh
		chmod +x ./devops/scripts/generate_topic_dinamic.sh
		./devops/scripts/generate_topic_dinamic.sh "${NAME_DIR_APP}" "${RELATIVE_SUBPATH_SRC}" "$baseFilename" "${ENV}"
...
#	  ls -lia ./automation
	  ...
	  chmod +x ./devops/scripts/generate_schema_registry_dinamic.sh
	  ./devops/scripts/generate_schema_registry_dinamic.sh "${NAME_DIR_APP}" "${RELATIVE_SUBPATH_SRC}" "$baseFilename" "${ENV}"
