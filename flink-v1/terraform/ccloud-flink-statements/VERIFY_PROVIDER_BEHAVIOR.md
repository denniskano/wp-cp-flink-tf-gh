...
La implementación actual usa `count` sobre listas `ddl_data` / `dml_data` cargadas desde YAML en orden `sort(fileset(...))` (índice = posición del archivo). Ventaja: simple. Inconveniente: quitar o reordenar un YAML reindexa recursos y puede provocar planes destructivos si no se migra el state.
