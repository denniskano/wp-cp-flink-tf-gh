...
output "ddl_statements" {
  ...
  value       = [for i, v in null_resource.ddl_statements : local.ddl_data[i]["statement-name"]]
  sensitive   = false
}

output "dml_statements" {
  description = "Lista de statements DML aplicados exitosamente"
  value       = [for i, v in null_resource.dml_statements : local.dml_data[i]["statement-name"]]
  sensitive   = false
}
...
output "all_statements" {
  ...
  value = concat(
    [for i, v in null_resource.ddl_statements : local.ddl_data[i]["statement-name"]],
    [for i, v in null_resource.dml_statements : local.dml_data[i]["statement-name"]]
  )
  sensitive = false
}
