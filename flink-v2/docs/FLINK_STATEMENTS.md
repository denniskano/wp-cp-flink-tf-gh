...
En Confluent Cloud el formato de registro suele ser `avro-registry` (no `avro-confluent`). El nombre de tabla es el nombre del topic; no hay propiedad `'kafka.topic'`.
...
En v2 el YAML esta en `PEVE-stream-processing-resources-v1`:

```
{CODAPP}/ccloud-flink/{desa|cert|prod}/{pipeline_flink}/statement/dml/*.yaml
```
...
DDL equivalente bajo `.../statement/ddl/*.yaml` (sin `stopped` en statements one-shot).
