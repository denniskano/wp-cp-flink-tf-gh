...
Custom SMT: [`smt.yaml`](smt.yaml) va en `{CODAPP}/{desa|cert|prod}/`, no en el use-case. Guía de uso (subir, pegar el `ca-…`, nueva versión, borrar): [docs/smt.md](docs/smt.md). SMT nativo no usa ese archivo.
...
Reemplaza host, topic, SA y paths de Vault. El topic, el schema y el SA tienen que existir antes del apply.
