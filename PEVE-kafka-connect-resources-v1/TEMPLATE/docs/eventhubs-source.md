...
|---|---|---|
| `max.events` | `50` | Eventos por poll y por partición del hub. Máximo 499. Subilo si hay lag y el hub tiene mucho backlog. |
| `tasks.max` | — | Ideal: 1 task por partición del Event Hub. Más tasks que particiones no suma. |
| `azure.eventhubs.offset.type` | `OFFSET` | `OFFSET` (offset de Event Hubs) o `SEQ_NUM`. No lo cambies después del primer apply. |
| `producer.override.linger.ms` | — | Agrupa writes a Kafka si el hub entrega de a poco. |
| `producer.override.compression.type` | — | Compresión hacia Kafka. |

El consumer group del hub (`$Default` o uno propio) no lo compartas con otra app: el conector avanza el checkpoint. Si recreás el conector (cambio de `name` o del archivo YAML) se pierden offsets y `starting.position` vuelve a aplicar.
